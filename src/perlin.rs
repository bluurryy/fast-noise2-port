...
#[inline]
pub fn gen2([x, y]: [f32; 2], seed: i32, _: Perlin) -> f32 {
    let xs = floor(x);
    let ys = floor(y);
    ...
    0.579106986522674560546875
        * lerp(
            lerp(
                gradient_dot2(hash_primes2(seed, x0, y0), xf0, yf0),
                gradient_dot2(hash_primes2(seed, x1, y0), xf1, yf0),
                xs,
            ),
            lerp(
                gradient_dot2(hash_primes2(seed, x0, y1), xf0, yf1),
                gradient_dot2(hash_primes2(seed, x1, y1), xf1, yf1),
                xs,
            ),
            ys,
        )
...
#[inline]
pub fn gen3([x, y, z]: [f32; 3], seed: i32, _: Perlin) -> f32 {
    let xs = floor(x);
    let ys = floor(y);
    let zs = floor(z);
    ...
    0.964921414852142333984375
        * lerp(
            lerp(
                lerp(
                    gradient_dot3(hash_primes3(seed, x0, y0, z0), xf0, yf0, zf0),
                    gradient_dot3(hash_primes3(seed, x1, y0, z0), xf1, yf0, zf0),
                    xs,
                ),
                lerp(
                    gradient_dot3(hash_primes3(seed, x0, y1, z0), xf0, yf1, zf0),
                    gradient_dot3(hash_primes3(seed, x1, y1, z0), xf1, yf1, zf0),
                    xs,
                ),
                ys,
            ),
            lerp(
                lerp(
                    gradient_dot3(hash_primes3(seed, x0, y0, z1), xf0, yf0, zf1),
                    gradient_dot3(hash_primes3(seed, x1, y0, z1), xf1, yf0, zf1),
                    xs,
                ),
                lerp(
                    gradient_dot3(hash_primes3(seed, x0, y1, z1), xf0, yf1, zf1),
                    gradient_dot3(hash_primes3(seed, x1, y1, z1), xf1, yf1, zf1),
                    xs,
                ),
                ys,
            ),
...
#[inline]
pub fn gen4([x, y, z, w]: [f32; 4], seed: i32, _: Perlin) -> f32 {
    let xs = floor(x);
    let ys = floor(y);
    let zs = floor(z);
    let ws = floor(w);
