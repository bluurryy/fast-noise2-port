...
use super::{
    gradient_dot2, gradient_dot3, gradient_dot4, hash_primes2, hash_primes3, hash_primes4,
    masked_add, masked_inc, masked_sub, nmasked_add, nmasked_sub, nmul_add, primes,
};
...
#[inline]
pub fn gen2([x, y]: [f32; 2], seed: i32, _: Simplex) -> f32 {
    ...
    let n1 = gradient_dot2(
        hash_primes2(
            ...
            if i1 { j } else { j.wrapping_add(primes::Y) },
        ),
        x1,
        y1,
    );
    let n2 = gradient_dot2(
        hash_primes2(seed, i.wrapping_add(primes::X), j.wrapping_add(primes::Y)),
        x2,
        y2,
    );
...
#[inline]
pub fn gen3([mut x, mut y, mut z]: [f32; 3], seed: i32, _: Simplex) -> f32 {
    ...
    let n2 = gradient_dot3(
        hash_primes3(
            ...
            nmasked_add(k, primes::Z, k2),
        ),
        x2,
        y2,
        z2,
    );
    let n3 = gradient_dot3(
        hash_primes3(
            seed,
            i.wrapping_add(primes::X),
            j.wrapping_add(primes::Y),
            k.wrapping_add(primes::Z),
        ),
        x3,
        y3,
        z3,
...
#[inline]
pub fn gen4([mut x, mut y, mut z, mut w]: [f32; 4], seed: i32, _: Simplex) -> f32 {
    ...
    let mut t0 = nmul_add(
        x0,
        x0,
        nmul_add(y0, y0, nmul_add(z0, z0, nmul_add(w0, w0, 0.6))),
    );
    let mut t1 = nmul_add(
        x1,
        x1,
        nmul_add(y1, y1, nmul_add(z1, z1, nmul_add(w1, w1, 0.6))),
    );
    let mut t2 = nmul_add(
        x2,
        x2,
        nmul_add(y2, y2, nmul_add(z2, z2, nmul_add(w2, w2, 0.6))),
    );
    let mut t3 = nmul_add(
        x3,
        x3,
        nmul_add(y3, y3, nmul_add(z3, z3, nmul_add(w3, w3, 0.6))),
    );
    let mut t4 = nmul_add(
        x4,
        x4,
        nmul_add(y4, y4, nmul_add(z4, z4, nmul_add(w4, w4, 0.6))),
    );
    ...
    27.0 * mul_add(
        n0,
        t0,
        mul_add(n1, t1, mul_add(n2, t2, mul_add(n3, t3, n4 * t4))),
    )
}
