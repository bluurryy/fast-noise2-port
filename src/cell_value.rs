use crate::{
    cell::{
        CellIndex, DistanceFn, JITTER_2D, JITTER_3D, JITTER_4D, MAX_DISTANCE_COUNT, calc_distance2,
        calc_distance3, calc_distance4,
    },
    hash_primes2_hb, hash_primes3_hb, hash_primes4_hb, inv_sqrt, mul_add, primes,
};
...
pub fn gen2([x, y]: [f32; 2], seed: i32, settings: CellValue) -> f32 {
    ...
    for _xi in 0..3 {
        ...
        for _yi in 0..3 {
            ...
            let mut new_distance = calc_distance2(settings.distance_fn, xd, yd);
            ...
            loop {
                ...
                if i >= settings.value_index as usize {
                    break;
                }
    ...
    value[settings.value_index as usize]
}

pub fn gen3([x, y, z]: [f32; 3], seed: i32, settings: CellValue) -> f32 {
    ...
    for _xi in 0..3 {
        ...
        for _yi in 0..3 {
            ...
            for _zi in 0..3 {
                ...
                let mut new_value = (1.0 / i32::MAX as f32) * hash as f32;
                let mut new_distance = calc_distance3(settings.distance_fn, xd, yd, zd);
...
pub fn gen4([x, y, z, w]: [f32; 4], seed: i32, settings: CellValue) -> f32 {
        ...
        for _yi in 0..3 {
            ...
            for _zi in 0..3 {
                ...
                for _wi in 0..3 {
                    ...
                    let inv_mag = jitter
                        * inv_sqrt(mul_add(xd, xd, mul_add(yd, yd, mul_add(zd, zd, wd * wd))));
                    xd = mul_add(xd, inv_mag, xcf);
                    yd = mul_add(yd, inv_mag, ycf);
                    zd = mul_add(zd, inv_mag, zcf);
                    ...
                    let mut new_value = (1.0 / i32::MAX as f32) * hash as f32;
                    let mut new_distance = calc_distance4(settings.distance_fn, xd, yd, zd, wd);
