...
use super::{
    gradient_dot2_fancy, gradient_dot3, hash_primes2, hash_primes3, masked_add, masked_sub,
    nmasked_add, nmasked_sub, nmul_add, primes,
};
...
#[inline]
pub fn gen2([x, y]: [f32; 2], seed: i32, _: OpenSimplex2) -> f32 {
    ...
    let n1 = gradient_dot2_fancy(
        hash_primes2(
            seed,
            masked_add(i, primes::X, i1),
            nmasked_add(j, primes::Y, i1),
        ),
        x1,
        y1,
    );
    let n2 = gradient_dot2_fancy(
...
#[inline]
pub fn gen3([x, y, z]: [f32; 3], mut seed: i32, _: OpenSimplex2) -> f32 {
    let f = (2.0 / 3.0) * (x + y + z);
    let mut xr = f - x;
    let mut yr = f - y;
    let mut zr = f - z;
    ...
    loop {
        ...
        let v1xr = masked_add(
            v0xr,
            f32::from_bits(f32::to_bits(1.0) | (f32::to_bits(-1.0) & d0xr.to_bits())),
            dir0xr,
        );
        let v1yr = masked_add(
            v0yr,
            f32::from_bits(f32::to_bits(1.0) | (f32::to_bits(-1.0) & d0yr.to_bits())),
            dir0yr,
        );
        let v1zr = masked_add(
            v0zr,
            f32::from_bits(f32::to_bits(1.0) | (f32::to_bits(-1.0) & d0zr.to_bits())),
            dir0zr,
        );
        let d1xr = xr - v1xr;
        let d1yr = yr - v1yr;
